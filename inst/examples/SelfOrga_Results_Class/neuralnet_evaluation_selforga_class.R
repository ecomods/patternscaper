...
#result files
eval_dir <- "inst/examples/SelfOrga_Results_Class/Results/"
fig_dir <- "inst/examples/SelfOrga_Results_Class/Figs/"
...
#calculate mean, sd and numer of na's and results
df_summary <- df_raw %>%
...
make_acc_mean_plot_all <- function(df) {

  df_plot <- df %>%
    mutate(
      training_size_num = as.numeric(gsub("[^0-9]", "", training_size))
    ) %>%
    arrange(training_size_num)

  # separate number of neurons for each input
  df_plot <- df_plot %>%
    group_by(inputmetrics) %>%
    mutate(neurons = factor(neurons, levels = sort(unique(neurons)))) %>%
    ungroup()

  ggplot(df_plot, aes(x = neurons, y = training_size_num, fill = mean_accuracy)) +
    ...
    scale_y_continuous(
      breaks = sort(unique(df_plot$training_size_num)),
      labels = sort(unique(df_plot$training_size_num))
    ) +
...
plot_acc_all <- make_acc_mean_plot_all(df_summary)
plot_acc_all

ggsave(
  filename = paste0(fig_dir,"plot_mean_accuracy_selforga.jpg",sep=""),
  plot = plot_acc_all,
  width = 5.5,
  height = 5,
  dpi = 300
...
#make heat meaps for mean accuracy
make_acc_sd_plot_all <- function(df) {
  ...
  ggplot(df_plot, aes(x = neurons, y = training_size_num, fill = sd_accuracy)) +
    geom_tile(color = "white") +
    facet_grid(
      metric ~ inputmetrics,
      scales = "free_x",
...
#show plots
plot_acc_sd_all <- make_acc_sd_plot_all(df_summary)
plot_acc_sd_all

ggsave(
  filename = paste0(fig_dir,"plot_sd_accuracy_selforga.jpg",sep=""),
  plot = plot_acc_sd_all,
  width = 5.5,
  height = 5,
  dpi = 300
)
...
#----------------------------------------------------------------
#----------------------------------------------------------------
#old - still has layers on y-axis and
#different plots for training size
#----------------------------------------------------------------
#----------------------------------------------------------------
