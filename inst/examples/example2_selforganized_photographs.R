...
for (i in 1:length(pic_names)) {
  ...
  ggsave(
    filename = paste0(
      "inst/examples/SelfOrga_Results_Class/Picture_",
      pic_names[i],
      ".jpg",
      sep = ""
    ),
  ...
  test_classification <- apply_nn_metrics(
...
for (i in 1:length(pic_names)) {
  print(paste(
    list_pic_results[[i]]$filename,
    ": ",
    list_pic_results[[i]]$predicted,
    " (",
    round(list_pic_results[[i]]$confidence, 3),
    ")",
    sep = ""
  ))
}
...
pic_landscape_class_metrics <- pic_landscape_class_metrics_all %>%
  filter(class == metric_class)
