...
#| fig-width: 8
#| fig-height: 5
#| fig-alt: "Eight spot and sharp landscapes arranged in four columns"
...
The function returns a [patchwork object](https://patchwork.data-imaginist.com/), which combines several `ggplot2` plots. Use `&` to add the same `ggplot2` layer to every panel. With multiple landscapes, `+` modifies only the last panel. Here, a vertical sampling transect is added at `x = 50`.

```{r}
#| label: add-transect
#| fig-width: 8
#| fig-height: 5
#| fig-alt: "Eight spot and sharp landscapes, each crossed by an orange dashed vertical transect"
