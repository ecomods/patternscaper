...
classified remote-sensing data. Users define the pattern classes
according to their research question and provide representative,
labelled training landscapes. These landscapes can be generated with
patternscaper or imported from ecological simulation outputs or other
categorical raster data. Artificial landscapes are particularly useful
when labelled empirical training data are scarce because their classes
are known by construction and their variation can be controlled.

The package provides two complementary workflows. The metric-based
workflow calculates established landscape metrics using the
[`landscapemetrics` R
...
them. The pixel-based workflow trains a convolutional neural network
directly on categorical raster cells without prior feature selection.
Both workflows support landscape preparation, model training and
