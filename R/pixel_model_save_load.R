...
#' \code{\link{apply_pixel_model}}, including class names and input dimensions.
#' Move or archive the complete folder rather than the single files inside it.
#'
...
validate_pixel_model_wrapper <- function(nn_model) {
  required <- c("model", "classes", "input_shape")
  if (
    !is.list(nn_model) ||
...
validate_pixel_model_bundle_metadata <- function(bundle_metadata, path) {
  ...
  required <- c("classes", "input_shape")
