...
for(t in 1:length(type)){
  all_c_indices_all_landscapes$type[str_detect(all_c_indices_all_landscapes$landscape,type[t])] <- type[t]
}
...
#----------------------------------------------------
evaluate_landscape_metrics <- function(calculated_metrics, metrics_list = "ALL",
                                       metrics_number = 10, method = "coeffvar_all"){
  ...
  means_types <- data.frame(tapply(calculated_metrics$value,list(as.factor(calculated_metrics$metric),as.factor(calculated_metrics$type)),mean, na.rm=T))
  means_types$all <- tapply(calculated_metrics$value,as.factor(calculated_metrics$metric),mean, na.rm=T)
  sd_types <- data.frame(tapply(calculated_metrics$value,list(as.factor(calculated_metrics$metric),as.factor(calculated_metrics$type)),sd, na.rm=T))
  sd_types$all <- tapply(calculated_metrics$value,as.factor(calculated_metrics$metric),sd, na.rm=T)
  ...
  if (method == "coeffvar_all") {
    ...
    top_metrics <- all_c_functions$metric[ranking>(num_metrics-metrics_number)] #take only top x
  }
  ...
  if (method == "linmod") {
    ...
    for(i in 1:num_metrics){
      dat <- all_c_indices_all_landscapes[all_c_indices_all_landscapes$metric==all_c_functions$metric[i],]
...
#test function
evaluate_landscape_metrics(calculated_metrics = all_c_indices_all_landscapes,metrics_number = 15, method = "coeffvar_all")
evaluate_landscape_metrics(calculated_metrics = all_c_indices_all_landscapes,metrics_number = 15, method = "linmod")
evaluate_landscape_metrics(calculated_metrics = all_c_indices_all_landscapes,metrics_number = 15, method = "mean_groups")
