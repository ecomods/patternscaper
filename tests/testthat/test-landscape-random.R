...
test_that("create_landscape_random validates veg_prob parameter", {
  # Valid veg_prob
  ...
  # Invalid veg_prob - non-numeric
  expect_error(
    create_landscape_random(width = 50, height = 50, veg_prob = "half"),
    "`veg_prob` must be between 0 and 1.",
    info = paste("Testing", name, "with non-numeric veg_prob")
  )

  # Invalid veg_prob - negative
  expect_error(
    create_landscape_random(width = 50, height = 50, veg_prob = -0.1),
    "`veg_prob` must be between 0 and 1.",
    info = paste("Testing", name, "with negative veg_prob")
  )

  # Invalid veg_prob - greater than 1
  expect_error(
    create_landscape_random(width = 50, height = 50, veg_prob = 1.5),
    "`veg_prob` must be between 0 and 1.",
    info = paste("Testing", name, "with veg_prob > 1")
  )
})
...
test_that("create_landscape_random veg_prob affects density", {
  set.seed(123)
...
test_that("create_landscape_random stores all params correctly", {
  ...
  expect_equal(l$params$veg_prob, 0.65)
...
test_that("create_landscape_random handles edge case veg_prob values", {
  # All vegetation
...
test_that("create_landscape_random works with extreme parameter combinations", {
  ...
  # Very large non-square landscape with extreme veg_prob
